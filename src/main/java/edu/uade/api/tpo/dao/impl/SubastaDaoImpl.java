...
public class SubastaDaoImpl extends AbstractManyToOneDao<Subasta> {
	...
	public PreparedStatement update(Subasta subasta, Connection conn) throws SQLException {
		...
		String query = "UPDATE " + schema
				+ ".subastas SET usuario_id = ?, fecha_desde = ?, fecha_hasta = ?, precio = ?, estado = ?, articulo_id = ?, comision = ?, precio_min = ?, dias_vigencia = ?, precio_inicial = ? where subasta_id = ?";
		PreparedStatement ps = conn.prepareStatement(query);
	...
	@Override
	public void delete(Subasta t) throws SQLException {
		throw new UnsupportedOperationException("Delete is not supported on class Subasta!");
	}
}
