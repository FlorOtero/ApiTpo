...
public class ServicioDaoImpl extends AbstractDao<Servicio> {
	...
	public PreparedStatement update(Servicio servicio, Connection conn) throws SQLException {
		String query = "UPDATE " + schema
				+ ".servicios SET nombre = ?, descripcion = ?, imagenes = ?, certificados = ?, tipo_contratacion = ? WHERE servicio_id = ?";
		PreparedStatement ps = conn.prepareStatement(query);
	...
	@Override
	public void delete(Servicio t) throws SQLException {
		try (Connection conn = this.getConnection(); PreparedStatement ps = getDeleteStatement(t, conn)) {
			ps.execute();
		}
	}

	private PreparedStatement getDeleteStatement(Servicio servicio, Connection conn) throws SQLException {
		String query = "DELETE FROM " + schema + ".servicios WHERE servicio_id = ?";
		PreparedStatement ps = conn.prepareStatement(query);
		ps.setString(1, servicio.getId());
		return ps;
	}
